  ...
  export default {
    get(path, cb, timeout) {
       const results =  get(path).then(res => {
         setTimeout(() => cb(res), timeout || TIMEOUT);
       });
    },
    findWhere(path, $where, cb , timeout){
      const results =  getWhere(path + $where).then(res => {
        setTimeout(() => cb(res), timeout || TIMEOUT);
      });
    },
    new(path, obj, cb, timeout) {
      const results =  add(path, obj).then(res => {
        setTimeout(() => cb(res), timeout || TIMEOUT);
      });
    },
    getOne(path, id, cb, timeout){
      const results =  getOne(path, id).then(res => {
        setTimeout(() => cb(res), timeout || TIMEOUT);
      });
    },
    modify(path, obj, cb, timeout){
      const results = update(path , obj.id, obj).then(res => {
        setTimeout(() => cb(res), timeout || TIMEOUT);
      });
    },
    delete(path, id, cb, timeout){
      const results = deleteData(path, id).then(res => {
        setTimeout(() => cb(res), timeout || TIMEOUT);
      });
    }
  };
